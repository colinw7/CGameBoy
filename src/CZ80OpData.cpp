...
#ifdef GAMEBOY_Z80
  else if (type == A_PI_REG) {
    str += "(" + CZ80::getRegisterName(arg) + "I)";
  }
  else if (type == A_PD_REG) {
    str += "(" + CZ80::getRegisterName(arg) + "D)";
  }
#endif
...
#ifdef GAMEBOY_Z80
  else if (type == A_P_FF00) {
    if      (arg == 1)
      str += "(0xff00 + 0x" + CStrUtil::toHexString(args[0], 2) + ")";
    else if (arg == 2) {
      ushort s = (args[1] << 8) | args[0];

      str += "(0xff00 + 0x" + CStrUtil::toHexString(s, 4) + ")";
    }
  }
#endif
